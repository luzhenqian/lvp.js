"object" == typeof exports && (module.exports = Lv); // 兼容 node CommonJS-like 规范
function Lv(lvOption) {
  // 检测运行环境
  var env,
  ...
  if (env === envType.amd) define.amd && define(Lv); // 兼容 AMD 规范
  // 工具函数
  ...
  var isBrowser = envType.browser === env || envType.amd === env,
    info = function(message) {
      ...
      restParams.unshift(
        isBrowser ? "%c lv info: %c " + message : "lv info: " + message,
    ...
    warn = function(message) {
      console.warn(
        isBrowser ? "%c lv warn: %c " + message : "lv warn: " + message,
  ...
  // 配置对象
  !lvOption && (lvOption = {});
  this.defaultSuccessMessage = lvOption.defaultSuccessMessage || "校验成功";
  this.defaultErrorMessage = lvOption.defaultErrorMessage || "校验失败";
  if (Array.isArray(lvOption.rules)) {
    for (var i = 0; i < lvOption.rules.length; i++) {
      this.addRules(lvOption.rules[i]);
    }
  }
   ...
   * @return {boolean}
   */
  Lv.prototype.required = function(value) {
   ...
   * @return {boolean}
   */
  Lv.prototype.min = function(value, minVal) {
   ...
   * @return {boolean}
   */
  Lv.prototype.max = function(value, maxVal) {
    if (typeof value === "number") {
      return value < maxVal;
    } else if (typeof value === "string") {
   ...
   * @return {boolean}
   */
  Lv.prototype.isEmail = function(value) {
   ...
   * @return {boolean}
   */
  Lv.prototype.isCNIDCard = function(value) {
   ...
   * @return {boolean}
   */
  Lv.prototype.isHttpURL = function(value) {
   ...
   * @return {boolean}
   */
  Lv.prototype.isImage = function(value) {
   ...
   * @return {boolean}
   */
  Lv.prototype.isCNPhone = function(value) {
   ...
   * @return {boolean}
   */
  Lv.prototype.isCNTEL = function(value) {
   ...
   * @return {boolean}
   */
  Lv.prototype.isCNChar = function(value) {
